  ...
  return (
    <div className={`rules-wrapper ${width < 1050 ? "w-100" : "w-78"}`}>
      ...
      <p className="rules-value">
        ...
        موعد تعیین شده اطمینان حاصل کند. در حال حاضر حداقل قیمت پیشنهادی برای هر
        صفحه ۱,۵۵۵ تومان و کارمزد تایپ‌کافه ۱۰٪ می‌باشد. که با توجه به درصد ذکر
        شده، تایپیست به ازای هر صفحه ۱,۴۰۰ تومان دریافتی، و کارفرما ۱,۷۱۱ تومان
        پرداختی خواهد داشت. بعد از تایید پیشنهاد تایپیست توسط کارفرما، مجموع
        مبلغ پروژه (قیمت پیشنهادی به ازای هر صفحه x تعداد صفحات ± کارمزد
        تایپ‌کافه) از اعتبار هر دو طرف کسر و نزد تایپ‌کافه گروگذاری خواهد شد.
        این مبلغ جهت حسن انجام کار توسط تایپیست و حصول اطمینان از پرداخت دستمزد
        توسط کارفرما از اعتبار طرفین اخذ می‌گردد و درصورت بروز هرگونه مورد عدم
        انطباق به شرح ذیل، وجه دریافتی به اعتبار طرف دیگر منتقل خواهد شد.
      </p>
      <div className="rules-value less-mt" style={{ margin: "10px 40px" }}>
        <p>
          ...
          اگر مهلت انجام پروژه ای یک ساعت باشد، در صورت عدم تحویل موفق پروژه پس
          از ۶۰ دقیقه، ابتدا بلافاصله، و سپس بعد از هر ۶ دقیقه تاخیر، ۱۰٪ از
          مبلغ گروگذاری شده به حساب کارفرما منتقل خواهد شد. در نتیجه پس از گذشت
          ۶۰ دقیقه از مهلت تعیین شده و ناموفق بودن تایپیست در تحویل پروژه در این
          بازه، کل مبلغ گروگذاری شده توسط تایپیست به حساب کارفرما منتقل، و یک
          پروژه ناموفق در پروفایل تایپیست ثبت خواهد شد.
        </p>
        <p>
          ۲. در صورت عدم پرداخت دستمزد تایپیست تا ۲۴ ساعت پس از تحویل نهایی
      ...
      <p className="rules-value less-mt">
        ...
        موردی، پروژه معلق خواهد شد. (در صورت ثبت پیشنهاد توسط تایپیست و تایید
        شدن آن، مسئولیت انجام پروژه بر عهده تایپیست می‌باشد و پروژه معلق نخواهد
        شد.) همچنین تایپیست به هیچ عنوان نمی‌بایست پروژه را ناقص تحویل دهد. در
        صورت گزارش و یا مشاهده چنین موردی تمام مبلغ گروگذاری شده توسط تایپیست بی
        چون و چرا به اعتبار کارفرما منتقل، و یک پروژه ناموفق در پروفایل تایپیست
        ثبت خواهد شد.
      </p>
      <p className="rules-value less-mt">
        در صورت بروز هرمشکلی پس از تایید پیشنهاد و به اجرا رفتن پروژه، و یا پس
        از به اتمام رسیدن پروژه، با گزارش پروژه تایپ‌کافه موضوع را بررسی و سپس
        رای نهایی را صادر می‌کند.
      </p>
      <p className="rules-value less-mt">
      ...
      <p className="rules-value">
        تایپ‌کافه تلاش کرده است تا کاربران نتوانند از رمز عبور ساده استفاده
        کنند، اما ممکن است کاربران از رمزهای عبور متداول استفاده کنند یا در حفظ
        و نگه‌داری رمز عبور خود دقت کافی را نداشته باشند؛ در این موقعیت با
        سواستفاده از آن رمز عبور ممکن است حساب کاربر مورد نفوذ قرار بگیرد. در
        این موقعیت تایپ‌کافه هیچ‌گونه مسئولیتی را قبول نمی‌کند. تایپ‌کافه به‌شدت
        تاکید می‌کند از رمز عبورهای پیچیده استفاده کنید و از انتشار آن به‌شکل
        مجازی و فیزیکی بپرهیزید.
      </p>
    </div>
  );
};
