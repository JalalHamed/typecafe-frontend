...
const Rules = () => {
  ...
  return (
    <div className={`rules-wrapper ${width < 1050 ? "w-100" : "w-78"}`}>
      ...
      <p className="rules-value">
        در کنار ساختار بسیار قدرتمند و امن تایپ‌کافه که با بهره‌گیری از
        فناوری‌های به‌روز دنیای تکنولوژی طراحی شده، و همچنین استفاده از پروتکل
        امن ارسال درخواست <i>https</i>، تایپ‌کافه تلاش کرده است تا کاربران
        نتوانند از رمز عبور ساده استفاده کنند. اما ممکن است کاربران از رمزهای
        عبور متداول استفاده و یا در حفظ و نگه‌داری رمز عبور خود دقت کافی را
        نداشته باشند؛ در این موقعیت با سواستفاده از رمز عبور ممکن است حساب کاربر
        مورد نفوذ قرار بگیرد. در این موقعیت تایپ‌کافه هیچ‌گونه مسئولیتی را قبول
        نمی‌کند. تایپ‌کافه به‌شدت تاکید می‌کند از رمز عبورهای پیچیده استفاده
        کنید و از انتشار آن به‌شکل مجازی و فیزیکی بپرهیزید.
      </p>
    </div>
  );
};
